    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 bg-white shadow sm:rounded-lg">
                <form class="mt-6 space-y-6" method="POST" action="{{ route('duty.store') }}">
                    {{ csrf_field() }}
                    ...
                    <div>
                        ...
                        <select name="dutyType" id="dutyType" class="form-input">
                            ...
                            <option value="Collecting Leftover Food">Collecting Leftover Food</option>
                            <option value="Repackaging Leftover Food">Repackaging Leftover Food</option>
                            <option value="Weighting Leftover Food">Weighting Leftover Food</option>
                            <option value="Distribute Leftover Food">Distribute Leftover Food</option>
                        </select>
                        @error('dutyType')
                    ...
                    <div>
                        ...
                        <select name="bazaarName" class="form-input" id="bazaarName">
                            ...
                            @foreach ($bazaarNames as $bazaarName)
                                <option value="{{ $bazaarName }}">{{ $bazaarName }}</option>
                            @endforeach
                        </select>
                        @error('bazaarName')
                            <span class="invalid-feedback" role="alert">
                    ...
                    <div>
                        ...
                        <x-text-input id="dutyRemarks" name="dutyRemarks" type="text" :value="old('dutyRemarks')" autofocus
                            class="mt-1 block w-full" />
                    ...
                    <div class="flex items-center gap-4">
                        <x-primary-button>{{ __('Create Duty') }}</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
